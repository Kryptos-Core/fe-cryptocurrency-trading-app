...
| DB, transaction, ORM, Redis/MQ (server) | **Repo backend riêng** — rule chi tiết nằm ở đó; trong repo Flutter chỉ tham chiếu qua API contract / `api-design` |
| Phân trang offset / keyset / cursor, sort, index list API | `pagination-best-practices.mdc` |
| Flutter Atomic + FSD ánh xạ repo | `flutter-fe-atomic-fsd.mdc` |
...
- Tái sử dụng: widget/component nội bộ → pub.dev ổn định trước khi viết mới; nhất quán UI/UX. Cấu trúc màn/feature → `flutter-fe-atomic-fsd.mdc`.

## Back-end (NestJS)

- Repo này là **Flutter client**; không sửa trực tiếp Nest trong workspace này. Khi cần đổi API, phối hợp **repo backend** của team BE và giữ contract (OpenAPI, versioning) — xem `api-design-architecture-patterns.mdc`.
...
3. **Đề xuất kỹ thuật**: map UI Flutter + contract API backend (repo riêng), reuse trong `lib/`, rồi triển khai.
