...
Future<CurrencyModel?> showCurrencyPickerBottomSheet(
...
class _CurrencyPickerBodyState extends State<_CurrencyPickerBody> {
  ...
  CurrencyModel? _currencyForRef(CurrencyRef ref) {
  ...
  Future<void> _pick(CurrencyModel c) async {
  ...
  Future<void> _toggleFavorite(CurrencyModel c) async {
    final store = widget.bookmarkStore;
    if (store == null) return;
  ...
  List<CurrencyModel> get _baseList {
    switch (_filterKind) {
      ...
      case _filterNonTradable:
        return widget.currencies.where((c) => !c.isTradable).toList();
      default:
        return List<CurrencyModel>.from(widget.currencies);
    }
  }
