...
class WalletsProvider extends ChangeNotifier {
  ...
  Future<void> fetchWalletBalance(String currencyId,
    ...
    _debugLog('[WalletsProvider] Fetching wallet balance');
    ...
    result.fold(
      (failure) {
        _debugLog(
            '[WalletsProvider] Wallet balance fetch failed: ${failure.runtimeType}');
        _error = _mapFailureToMessage(failure);
        _isLoading = false;
        notifyListeners();
      },
      (balance) {
        _debugLog('[WalletsProvider] Wallet balance fetched successfully');
  ...
  Future<bool> adminAdjustBalance({
    ...
    result.fold(
      ...
      (adjustment) {
        _debugLog('[WalletsProvider] Admin adjustment created successfully');
  ...
  void _subscribeToWalletBalanceStream() {
    ...
    _walletBalanceSubscription =
      ...
      onError: (error) {
        _debugLog('[WalletsProvider] Wallet balance stream error');
      },
    );
    _debugLog('[WalletsProvider] Subscribed to wallet balance stream');
  }
  ...
  void _handleWalletBalanceEvent(WalletBalanceEvent event) {
    ...
    if (event.currencyId == _walletBalance!.currencyId) {
      _debugLog('[WalletsProvider] Real-time wallet balance update received');
