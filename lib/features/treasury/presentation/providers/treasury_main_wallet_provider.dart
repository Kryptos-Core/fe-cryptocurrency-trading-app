...
class TreasuryMainWalletProvider extends ChangeNotifier {
  ...
  Future<void> loadMainWallets() async {
    ...
    try {
      final list = await _repository.listMainWallets(_currentChain);
      _mainWallets = list;
    } catch (e) {
      _error = e.toString();
    } finally {
  ...
  Future<void> loadPendingWallets() async {
    ...
    try {
      final list = await _repository.listPendingMainWallets();
  ...
  Future<void> refreshAllWallets() async {
    ...
    try {
      final main = await _repository.listMainWallets(_currentChain);
      _mainWallets = main;
      if (_includePendingWallets) {
        final pending = await _repository.listPendingMainWallets();
  ...
  Future<bool> importMainWallet({
    ...
    try {
      await _repository.importMainWallet(
        chain: _currentChain,
        label: label,
  ...
  Future<bool> approveWallet(String id) async {
    ...
    try {
      await _repository.approveMainWallet(id);
      await refreshAllWallets();
      return true;
    } catch (e) {
      _error = e.toString();
  ...
  Future<bool> rejectWallet(String id) async {
    ...
    try {
      await _repository.rejectMainWallet(id);
      await refreshAllWallets();
      return true;
    } catch (e) {
      _error = e.toString();
  ...
  Future<bool> setDefaultWallet(String id) async {
    ...
    try {
      await _repository.setDefaultMainWallet(id);
      await refreshAllWallets();
      return true;
    } catch (e) {
      _error = e.toString();
  ...
  Future<String?> revealMainWalletPrivateKey({
    ...
    try {
      final pk = await _repository.revealMainWalletPrivateKey(
  ...
  Future<bool> updateMainWalletLabel(String mainWalletId, String? label) async {
    ...
    try {
      await _repository.updateMainWallet(
        mainWalletId: mainWalletId,
        label: label,
      );
      await refreshAllWallets();
  ...
  Future<bool> requestMainWalletDeletion(String mainWalletId) async {
    ...
    try {
      await _repository.requestMainWalletDeletion(mainWalletId);
      await refreshAllWallets();
      return true;
    } on ServerException catch (e) {
      _error = e.message;
  ...
  Future<bool> approveMainWalletDeletion(String mainWalletId) async {
    ...
    try {
      await _repository.approveMainWalletDeletion(mainWalletId);
      await refreshAllWallets();
      return true;
    } catch (e) {
      _error = e.toString();
  ...
  Future<bool> rejectMainWalletDeletion(String mainWalletId) async {
    ...
    try {
      await _repository.rejectMainWalletDeletion(mainWalletId);
      await refreshAllWallets();
      return true;
    } catch (e) {
      _error = e.toString();
