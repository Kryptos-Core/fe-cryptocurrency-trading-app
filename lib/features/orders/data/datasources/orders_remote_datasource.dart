...
///
/// Handles HTTP calls, response parsing, and BE error mapping.
abstract class OrdersRemoteDataSource {
  ...
  Future<({List<OrderModel> data, int total, int page, int limit})>
      getMyOrders({
    int page = 1,
    int limit = 20,
    String? status,
...
class OrdersRemoteDataSourceImpl implements OrdersRemoteDataSource {
    ...
    try {
      ...
      if (envelope is Map<String, dynamic>) {
        ...
        if (inner is List) {
          ...
          pageVal =
              _toInt(envelope['page']) == 0 ? page : _toInt(envelope['page']);
          limitVal = _toInt(envelope['limit']) == 0
              ? limit
              : _toInt(envelope['limit']);
        }
  ...
  Exception _mapDioToException(DioException e) {
    ...
    return ServerException(
        message: message.isNotEmpty ? message : e.message ?? 'Network error');
  }
