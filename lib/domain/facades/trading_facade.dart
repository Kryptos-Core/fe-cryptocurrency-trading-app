...
class TradingFacade {
  ...
  final WalletRepository
      _walletRepository; // ignore: unused_field — reserved for balance checks
  ...
  Future<Either<Failure, Order>> placeMarketOrder({
    ...
    final marketsResult =
        await _marketsRepository.getMarkets(search: marketId, limit: 1);

    return marketsResult.fold((failure) => Left(failure), // Forward failure
        (markets) async {
